  ...
  return (
    <nav
      className={`panel-menu mobile-main-menu ${
        showMobileNavbar ? "mmitemopen" : null
      } ${className ? className : ""}`}
    >
        ...
        <div className="mmpanel mmopened mmcurrent" id="mm0">
          <ul>
            <li className="mm-close-parent">
              <button className="mm-close" onClick={mobileNavbarHandler}>
                Close
              </button>
            </li>
            {navData.map((nav) => (
              <li key={nav.id}>
                ...
                >
                  <span
                    onClick={(event) => event.stopPropagation()} // Prevents menu expansion on text click
